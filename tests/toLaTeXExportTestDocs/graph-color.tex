...
%%% Line 37
A \index{graph} is a functor \(G: \Gamma \to \Set\).   Consider, 
...
%%% Line 138
A \index{coloring} of a graph is a function \(c\) from its vertices to a set \(C\), the set of colors, such that vertices 
...
\(G\) is \index{n-colorable} if \(C_n(G)\) is non-empty.   A
...
functors. It follows that \(C_n\) is a \index{representable functor}.
